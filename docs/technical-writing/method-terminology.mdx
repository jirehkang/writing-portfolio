---
id: method-terminology
title: Method terminology
description: Codecademy  |  December 2024
slug: /method-terminology
sidebar_label: 'Method terminology'
sidebar_position: 5
---

import InteractiveButton from '@site/src/components/InteractiveButton'

# Method terminology

Codecademy  |  December 2024

![Markdown](https://img.shields.io/badge/Markdown-000000?style=for-the-badge&logo=markdown&logoColor=white) ![VSCode](https://img.shields.io/badge/VSCode-0078D4?style=for-the-badge&logo=visual%20studio%20code&logoColor=white) ![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white) 

## Project overview

I contributed to Codecademy’s open-source documentation by creating a terminology entry for the `.Join()` method in C#. 

<InteractiveButton url="https://www.codecademy.com/resources/docs/c-sharp/strings/join">View Project →</InteractiveButton>

*Note: All content was written by me.*

### Scope of work

- Wrote a clear, SEO-optimized definition of the `.Join()` method, focusing on its role in C# string manipulation.
- Included practical code examples showing how to join string arrays using different delimiters.

## Outcome

- Improved overall reference quality by adding a clear and concise explanation of the `.Join()` method’s purpose and syntax.
- Contributed to Codecademy Docs’ mission to provide reliable, beginner-friendly programming references.

![screenshot of C# .Join() method terminology page](./img/join-csharp-method.png)
